DNA Profiling Tool (CS50P Problem Set 6)

This Python script identifies a person based on their DNA sequence by analyzing Short Tandem Repeats (STRs). It simulates how forensic scientists use genetic databases to match evidence to individuals.

Key Technical Implementation:

  File I/O: Parses large CSV databases using csv.DictReader and reads raw genetic sequences from text files.

  Pattern Matching: Implements a sliding-window algorithm to find the longest consecutive run of specific DNA subsequences (STRs).

  Data Mapping: Compares computed STR counts against a list of dictionary objects to find a perfect match.

  Command-Line Interface: Uses sys.argv for robust handling of file inputs.

====== How to use ======

    :- First copy this whole folder in your local computer 
